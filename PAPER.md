# An elementary lower bound for binary twofold coverings of the Hamming cube

**Hanyu Yang** — ORCID [0009-0005-0419-4070](https://orcid.org/0009-0005-0419-4070)

*Draft, 2026-09-02. Not submitted. See §9 for what is and is not established.*

## Abstract

For even `n`, every binary code `C ⊆ F_2^n` whose closed radius-1 Hamming balls
cover each point of `F_2^n` at least twice satisfies

> **|C| ≥ ⌈ 3·2^(n+1) / (3n+2) ⌉.**

The proof is half a page: a parity refinement of the covering-excess method in
the tradition of Johnson and van Wee. The bound strictly improves the best
published lower bound on `K(n,1,2)` for **every even n ≥ 6**, and in particular
at the five positions `n = 8, 10, 12, 14, 16` tabulated by Krotov and Potapov
...
The method generalises to all even multiplicities `μ`, but improves on the
published bounds only at `μ = 2`; we say why.
...
1. an exact-rational SCIP → VIPR certificate refuting `|C| = 59`, accepted by
   the standalone checker `viprchk` (3.59 GB; see `CERTIFICATION.md`);
...
## 8. A weight-parity refinement, and the state of `M = 60`
...
**Established** — Theorems 1, 2, Corollary 3, Proposition 4, and
