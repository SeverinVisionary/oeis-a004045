...
\noindent
{\large \bf A parity refinement of the covering excess bound for twofold coverings of the hypercube}\\

\noindent
Hanyu Yang$^{1,}\footnote{Corresponding author (hanyu.yang.92@gmail.com)}$\\

\noindent
\footnotesize $^1${\it Independent researcher\/}\\
...
hypercube lies within Hamming distance one of at least two codewords, and
$K(n,1,2)$ denotes the least size of such a code. For every even $n$ we prove a
lower bound on $K(n,1,2)$ by a parity observation taken at a codeword rather
than at an arbitrary word: in a code without repeated words the ball of a
codeword carries a unique diagonal term, whose parity forces an excess whenever
$n$ is even. After taking ceilings the bound exceeds the general lower bound of
Krotov and Potapov for every even $n$ from six onwards. It raises the tabulated
lower bounds at the five lengths $n=8,10,12,14,16$, and at $n=6$ it supplies an
elementary lower-bound proof which, with the known twenty-word construction,
recovers the exact value. The argument extends to every even multiplicity, and
we determine exactly when the extension improves on the Krotov--Potapov bound
and when it does not.
\\[2mm]
{\bf Keywords:} covering code; multiple covering; twofold covering; covering excess; hypercube; lower bound.\\[2mm]
...
not a multiset. Allowing repetitions gives a different quantity, written
$\overline{K}$ in \cite{HHKL-1993}, and the hypothesis that $C$ is a set is used
essentially below. We write $K(n,1,2)$ for the twofold case.
...
Section \ref{sec-mu} carries the argument to every even multiplicity and
determines exactly when it improves on \cite{KP-2021}.
...
$g$ is a non-negative integer vanishing off $S$,
...
In $\mathbb{F}_2^3$ the code $C=\{000,001,110,111\}$ has $c(x)=2$ for every $x$,
...
Carrying a general $\mu$ through the proof, Step 1 requires the ball sum
$(n+1)+2N_2(x)$ to exceed $\mu(n+1)$ by rounding, which happens exactly when
$\mu(n+1)$ is even, that is, for even $n$, exactly when $\mu$ is even.

\begin{proposition}\label{prop-mu}
Let $n\ge 2$ be even and let $\mu$ be a positive even integer. Then
\[
...
$\varepsilon$ vanishes.\footnote{The authors of \cite{HHKL-1993} cite a
forthcoming manuscript of W.\ Chen and D.\ Li entitled \emph{Lower bounds for
multiple covering codes}. I have not located a published version, so the
novelty comparison here is made relative to the accessible published
literature.}

\section*{Acknowledgment}

The author used generative artificial-intelligence tools substantially in
developing and preparing this work, including for literature exploration, for
proposing candidate arguments, and for drafting text. The author independently
verified every mathematical statement and assumes full responsibility for the
content. No funding was received.
...
\bibitem{OEIS-A004045} OEIS Foundation Inc., Entry A004045, {\it The On-Line Encyclopedia of Integer Sequences}, \underline{https://oeis.org/A004045} (accessed 3 September 2026).
